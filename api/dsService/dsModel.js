...
const getUserByEmail = async (useremail) => {
  ...
  return db('profiles').where({ email: `${useremail}` }).first()
}
...
const getMoneyFlow = async (user_ID, time_period) => {
  return getUserByEmail(user_ID)
    .then(async (user) => {
      return await dsClient.post(`/moneyflow`, { user_ID: user.saverlife_profile_username, time_period })
    })
};
...
const getSpending = async (user_ID, graph_type, time_period) => {
  return getUserByEmail(user_ID)
    .then(async (user) => {
      return await dsClient.post('/spending', { user_ID: user.saverlife_profile_username, graph_type, time_period });
    })
};
...
const getCurrentMonthSpending = async (user_id) => {
  return getUserByEmail(user_id)
    .then(async user => {
      return dsClient.get(`/current_month_spending?user_id=${user.saverlife_profile_username}`);
    })
};
